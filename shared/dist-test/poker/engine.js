...
import { evaluateBest, compareScores } from './evaluator.js';
...
export class TableEngine {
    ...
    seatAt(i) {
        ...
        if (!s)
            throw new Error(`Bad seat index ${i}`);
        return s;
    }
    ...
    startHand(handId) {
        const ready = this.seats.filter((s) => s.state !== 'empty' && s.state !== 'sitting_out' && s.stack > 0n);
        if (ready.length < 2)
        ...
        for (const s of this.seats) {
            ...
            s.inHand = false;
            if (s.state === 'folded')
                s.state = s.userId ? 'waiting' : 'empty';
        }
    ...
    dealHoleCards() {
        ...
        while (true) {
            ...
            if (cur === this.nextLive(this.buttonSeat))
                break;
        }
    ...
    act(seatIndex, action) {
        ...
        switch (action.kind) {
            ...
            case 'raise': {
                if (this.currentBet === 0n)
                    throw new Error('Nothing to raise; use bet');
                const total = action.amount ?? 0n;
    ...
    advanceStreet() {
        ...
        const canActPlayers = this.seats.filter((s) => s.inHand && s.state !== 'folded' && s.state !== 'all_in');
    ...
    endHandUncontested() {
        ...
        winner.stack += award;
        if (rake > 0n)
            this.push({ kind: 'rake_taken', amount: rake });
        this.push({
            kind: 'pot_won',
    ...
    runShowdown() {
        ...
        if (totalRake > 0n)
            this.push({ kind: 'rake_taken', amount: totalRake });
        const awards = [];
        pots.forEach((pot, idx) => {
            ...
            for (const r of eligibleEvals) {
                if (!best || compareScores(r.eval.score, best.score) > 0)
                    best = r.eval;
            }
            const winners = eligibleEvals.filter((r) => compareScores(r.eval.score, best.score) === 0);
     ...
     */
    calcRake(pot, postflopShowdown) {
        if (pot < this.config.bigBlind * 2n)
            return 0n; // no rake on tiny pots
        if (!postflopShowdown && this.community.length === 0)
            return 0n; // no flop, no drop
    ...
    pots() {
        return buildPots(this.seats.map((s) => ({
            seat: s.index,
            contributed: s.totalBet,
            eligible: s.inHand && s.state !== 'folded',
        })));
