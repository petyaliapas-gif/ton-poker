import { TableEngine, cardCode } from '@ton-poker/shared';
...
export class Room {
    ...
    handsLastHour = 0;
    cfg;
    constructor(cfg, rakePercent, rakeCapBb) {
        this.id = cfg.id;
        this.cfg = cfg;
    ...
    canStartHand() {
        if (this.engine.street !== 'idle')
            return false;
        const ready = this.engine.seats.filter((s) => s.userId && s.state !== 'sitting_out' && s.stack > 0n).length;
        return ready >= 2;
    }
    maybeStartHand() {
    ...
    broadcastEvent(ev) {
        ...
        switch (ev.kind) {
            ...
            case 'street_advanced': {
                const cards = ev.community.map(cardCode);
        ...
        if (wireEvent) {
            const msg = { t: 'event', event: wireEvent };
