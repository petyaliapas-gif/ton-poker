...
import type { ClientMsg, ServerMsg } from '@ton-poker/shared';
 ...
 */
export function registerWebSocket(app: FastifyInstance, mgr: RoomManager): void {
  ...
  app.get('/ws', { websocket: true }, async (socket: WebSocket, req) => {
    ...
    try {
      ...
      socket.send(
        JSON.stringify({ t: 'error', code: 'AUTH', message: String(err) }),
      );
      socket.close();
      return;
    }
    ...
    room.attach({
      ...
      send,
      seatIndex:
        room.engine.seats.findIndex((s) => s.userId === user.id) ?? -1,
    });
    ...
    async function handle(msg: ClientMsg): Promise<void> {
      switch (msg.t) {
        case 'hello':
        ...
        case 'sit': {
          ...
          if (buyIn <= 0n) throw new Error('bad buyIn');
        ...
        case 'sit_out': {
          ...
          if (seatIdx >= 0) {
            ...
            if (s.state !== 'empty') s.state = 'sitting_out';
          }
          break;
        }
        case 'sit_in': {
          ...
          if (seatIdx >= 0) {
            const s = room!.engine.seatAt(seatIdx);
            if (s.state === 'sitting_out') s.state = 'waiting';
          }
          break;
        }
        case 'chat':
