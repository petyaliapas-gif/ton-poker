 ...
 */
export function registerStarsRoutes(app: FastifyInstance, bot: Bot, pricing: PricingService): void {
  ...
  app.post('/api/stars/invoice', async (req, reply) => {
    ...
    if (itemKind === 'cosmetic') {
      ...
      if (!item) return reply.code(404).send({ error: 'cosmetic not found' });
  ...
  app.get<{ Params: { id: string } }>('/api/tournaments/:id/share', async (req, reply) => {
    const t = await db.tournament.findUnique({ where: { id: req.params.id } });
    if (!t) return reply.code(404).send({ error: 'not found' });
    const kb = new InlineKeyboard().webApp(
      'Открыть в TON Poker',
      `${cfg.MINI_APP_URL}?startapp=t_${t.id}`,
    );
