...
export function registerAuthRoutes(app: FastifyInstance): void {
  ...
  app.post<{ Body: AuthBody }>('/api/auth', async (req, reply) => {
    try {
      const { initData } = req.body ?? ({} as AuthBody);
      ...
      if (!parsed.user) return reply.code(401).send({ error: 'no user in initData' });
      if (parsed.user.is_bot && !cfg.ALLOW_BOT_USERS) {
        return reply.code(403).send({ error: 'bots not allowed' });
      }

      const country = (req.headers['cf-ipcountry'] ?? req.headers['x-country']) as
        | string
        | undefined;
