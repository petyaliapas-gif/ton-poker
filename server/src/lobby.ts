...
import type { CashTableInfo, StakeLevel } from '@ton-poker/shared';

/** Static cash-table configuration (loaded once at startup). */
export interface CashTableConfig {
  id: string;
  name: string;
  stake: StakeLevel;
  smallBlind: bigint;
...
export const CASH_TABLES: CashTableConfig[] = [
  // Micro
...
export function findTable(id: string): CashTableConfig | undefined {
 ...
 */
export function publicTableInfo(
  cfg: CashTableConfig,
  dyn: { seated: number; waiting: number; avgPot: bigint; handsPerHour: number },
): CashTableInfo {
  return {
    id: cfg.id,
    name: cfg.name,
